...
La masa del recipiente donde caerá el hielo derretido es de $M_r=106.7$ g.

Ahora, para cada material problema, medimos tanto su espesor $h$ co o díametro $d_0$.

```{r}
# Datos recogidos
i <- c(1,2,3,4,5)
materiales <- c("Yeso", "Vidrio", "Madera", "Policarbonato", "Metacrilato")
d <- c(78.85,75,47.90,73.20,71.75) #mm
h <- c(5.85,6.10,6.30,6.35,6.15) #mm

# Tabla
data <- data.frame(i,materiales,d,h)
colnames(data) <- c("Medida","Materiales","d / mm","h / mm")
  
knitr::kable(
  data,
  caption="Espesores y diámetros de los distintos moldes de hielo. Masa de agua.",
  booktabs = TRUE,
  escape = FALSE,
  row.names = FALSE,
  align="c"
...
A partir de las masas de hielo derretidas en cada una de las experiencias, y el tiempo transcurrido, donde \(\Delta t = 5\) min para todas nuestras experiencias, calculamos la velocidad a la que se derrite en cada uno de los casos mediante la fórmula \(R_n = m_n / \Delta t\). Si además usamos la fórmula de propagación de errores \(\sigma^2 = \sum_i (\partial f / \partial x_i)^2 \sigma_i^2\), la expresión que tenemos que usar para calcular el error en la velocidad es:
...
```{r}
# Datos recogidos
mtot <- c(20.3,24.1,18.2,3.70,17.9)+106.7 #masa con recipiente, gramos
Mrec <- 106.7 #masa del recipiente, gramos
...
# Tabla
data <- data.frame(i,materiales,mtot,m,R,inc_R)
colnames(data) <- c("Medida","Materiales","$m_{tot}$ / g","m / g", "R / g/s", "$\\Delta R$ / g/s")
  
knitr::kable(
  data,
  caption="Masa de agua derretida con y sin contar la del recipiente de cada uno de los experimentos. Velocidades a las que se derriten",
  booktabs = TRUE,
  escape = FALSE,
  row.names = FALSE,
  align="c"
...
$$
\Delta k^2 = \Big( \frac{L_fh}{S\Delta \theta} \cdot \Delta R \Big)^2 + \Big( \frac{RL_f}{S\Delta \theta} \cdot \Delta h \Big)^2 + \Big( \frac{RL_fh}{S^2\Delta \theta} \cdot \Delta S \Big)^2 + \Big( \frac{RL_f}{S (\Delta \theta)^2} \cdot \Delta (\Delta \theta) \Big)^2
$$
donde usamos que como $S = \pi(\frac{d}{2})^2$, entonces $\Delta S = \frac{\pi}{2} d \Delta d$. Además, las incertidumbres en las longitudes las tomaremos como las de la precisión de la regla $\Delta h = \Delta d = 1$ mm, y como error en la subida de temperatura #TODO
...
term4 <- ( (R * Lf) / (S * (incr_theta)^2) * inc_incr_theta )^2
inc_k <- sqrt(term1 + term2 + term3 + term4)

# Tabla
data <- data.frame(i,materiales,k, inc_k)
colnames(data) <- c("Medida","Materiales","k / -","$\\Delta k$ / -")
  
knitr::kable(
  data,
  caption="Coeficientes de conductividad térmica para cada material",
  booktabs = TRUE,
  escape = FALSE,
  row.names = FALSE,
  align="c"
