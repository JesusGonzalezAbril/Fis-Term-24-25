...
La realizaremos, como se explicó anteriormente, a volumen constante 5 L.
...
knitr::kable(
  data,
  caption="Tiempo que se ha mantenido el circuito cerrado para hacer cada variación de presión",
  booktabs = TRUE,
  escape = FALSE,
  row.names = FALSE,
  align="c"
...
```{r, echo=FALSE,  out.width= "90%", fig.align='center', fig.cap="Ajuste de P sobre t"}
...
- Pendiente: $a = \frac{V_e I R}{ V \overline{C}_{V}} =`r pendiente`$

- Incertidumbre pendiente: $\Delta a =`r inc_pendiente`$

Expresándolo correctamente:
\[
a = `r round(pendiente,2)` \pm `r signif(inc_pendiente,2)`
\]

Despejando el valor de $C_v$ a partir de la pendiente como $\overline{C}_{V} = \frac{V_e I R}{ V a}$, obtenemos un valor de $\overline{C}_{V} =`r Cv`$. Si además usamos la fórmula de propagación de errores \(\sigma^2 = \sum_i (\partial f / \partial x_i)^2 \sigma_i^2\), la expresión que tenemos que usar para calcular el error es:

\begin{equation}
\Delta \overline{C}_{V} = \frac{V_e I R}{ V } \cdot \frac{\Delta a}{a^2}
\end{equation}

de donde obtenemos que $\Delta \overline{C}_{V} =`r inc_Cv`$. Expresándolo correctamente:

\[
C_v = `r round(Cv,2)` \pm `r signif(inc_Cv,2)`
\]
...
```{r, echo=FALSE,  out.width= "90%", fig.align='center', fig.cap="Ajuste de V sobre t"}
# Ajuste lineal
...
- Pendiente: $a = \frac{V_e I R}{ P \overline{C}_{P}} =`r pendiente2`$

- Incertidumbre pendiente: $\Delta a =`r inc_pendiente2`$

Expresándolo correctamente:
\[
a = `r round(pendiente2,2)` \pm `r signif(inc_pendiente2,2)`
\]

\clearpage

Despejando el valor de $C_P$ a partir de la pendiente como $\overline{C}_{P} = \frac{V_e I R}{ P a}$, obtenemos un valor de $\overline{C}_{P} =`r Cp`$. Si además usamos la fórmula de propagación de errores \(\sigma^2 = \sum_i (\partial f / \partial x_i)^2 \sigma_i^2\), la expresión que tenemos que usar para calcular el error es:

\begin{equation}
\Delta \overline{C}_{P} = \frac{V_e I R}{ P } \cdot \frac{\Delta a}{a^2}
\end{equation}

de donde obtenemos que $\Delta \overline{C}_{P} =`r inc_Cp`$. Expresándolo correctamente:

\[
C_P = `r round(Cp,2)` \pm `r signif(inc_Cp,2)`
\]
...
Como $R = \overline{C}_{P} - \overline{C}_{V}$, el error que estamos cometiendo es $\Delta R = \sqrt{\Delta \overline{C}_{P}^2 + \Delta \overline{C}_{V}^2}$. Expresándolo correctamente:

\[
R = `r round(Rexp,2)` \pm `r signif(inc_Rexp,2)` \ \frac{J}{mol \cdot K}
\]
