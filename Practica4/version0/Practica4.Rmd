...
Despejando de $C_v$ a partir de la pendiente como $\overline{C}_{V} = \frac{V_e I R}{ V a}$, obtenemos un valor de $\overline{C}_{V} =`r Cv`$. Si además usamos la fórmula de propagación de errores \(\sigma^2 = \sum_i (\partial f / \partial x_i)^2 \sigma_i^2\), la expresión que tenemos que usar para calcular el error es:
...
Despejando de $C_P$ a partir de la pendiente como $\overline{C}_{P} = \frac{V_e I R}{ P a}$, obtenemos un valor de $\overline{C}_{P} =`r Cp`$. Si además usamos la fórmula de propagación de errores \(\sigma^2 = \sum_i (\partial f / \partial x_i)^2 \sigma_i^2\), la expresión que tenemos que usar para calcular el error es:
...
Como $R = \overline{C}_{P} - \overline{C}_{V}$, el error que estamos cometiendo es $\Delta R = \sqrt{\Delta \overline{C}_{P}^2 + \Delta \overline{C}_{V}^2}$. Expresandolo correctamente:

\[
R = `r round(Rexp,2)` \pm `r signif(inc_Rexp,2)`
\]
...
MUCHISIMO ERROR EN LA SEGUNDA. FUENTES:
- JERINGUILLA, POLVILLO DE DUDOSA PROCEDENCIA. 
- MISMAS VARIACIONES DE PRESION DISTINTAS VARIACIONES DE PRESION EN EL MANOMETRO.