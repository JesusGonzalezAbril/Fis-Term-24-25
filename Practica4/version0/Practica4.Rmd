...
# Tabla
data <- data.frame(t,P)
colnames(data) <- c("tiempo","var presion")
  
knitr::kable(
  data,
  caption="tiempo y variacion presion",
  booktabs = TRUE,
  escape = FALSE,
  row.names = FALSE,
  align="c"
...
plot(t,P, main = titulo, ylab =expression(Y ~ (cm^{-2})), xlab =expression(X ~ (s^{-1})))
abline(modelo_lineal, col = "coral")

Cv <- voltaje*intensidad/(volumen*pendiente*200)
```
